...
class DirectMPIGO(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
        ...
        if rgbnet_dim <= 0:
            ...
            # dim0 = (3+3*viewbase_pe*2) + self.k0_dim # 12
            dim0 = (3+3*viewbase_pe*2) + 3 + self.k0_dim # 15 # 3 for position
            ...
            # added 
            nn.init.constant_(self.rgbnet[0].weight[:, -(3+3*viewbase_pe*2):], 0)
            # nn.init.constant_(self.rgbnet[0].bias, 0)
        ...
        # self.iter_with_max_view_embedding=30000 # max 60k
    ...
    def sample_ray(self, rays_o, rays_d, near, far, stepsize, use_mask=True, **render_kwargs):
        ...
        if self.training:        
            ray_pts_full, mask_outbbox = self.volume_sampling_ndc(rays_o, rays_d, self.xyz_min, self.xyz_max, N_samples)        
        else:
            ray_pts, mask_outbbox = render_utils_cuda.sample_ndc_pts_on_rays(rays_o, rays_d, self.xyz_min, self.xyz_max, N_samples)
        mask_inbbox = ~mask_outbbox
        if use_mask:                        
            mask_inbbox_ = mask_inbbox.unsqueeze(-1).repeat(1,1,3)
            # ray_pts = ray_pts_full[mask_inbbox_]            
            ray_pts=torch.masked_select(ray_pts_full, mask_inbbox_).reshape(-1,3)            
            if mask_inbbox.all():
            ...
            mask_inbbox = torch.ones_like(mask_inbbox, dtype=torch.bool)            
            mask_inbbox_ = mask_inbbox.unsqueeze(-1).repeat(1,1,3)
            # ray_pts = ray_pts_full[mask_inbbox_]            
            ray_pts=torch.masked_select(ray_pts_full, mask_inbbox_).reshape(-1,3)            
            ray_id, step_id = create_full_step_id(mask_inbbox.shape)
...
# added -----------------------------------------------------------------------------------------------------------
    # def volume_sampling_ndc(self, c2w, ray_dir_cam, t_vals, near, far, H, W, focal, perturb_t):
    def volume_sampling_ndc(self, rays_o, rays_d, xyz_min, xyz_max, N_samples):        
        """
        ...
        """
        # print('hello') # dist = ((float)i_step) / (N_samples-1);
        # ray_pts.shape
        # torch.Size([72, 255, 3])
        # mask_outbbox.shape
        # torch.Size([72, 255])
        ...
        rays_o = rays_o.repeat(1, N_samples, 1) # .clone()
        rays_d = rays_d.reshape(-1, 1, 3)
        rays_d = rays_d.repeat(1, N_samples, 1)
        t = torch.arange(0, N_samples, 1)/(N_samples-1)
        t = t.reshape(1, N_samples, 1)
        ray_pts = rays_o + rays_d*t
        
        # ray_pts = torch.zeros((n_rays, N_samples, 3), dtype = rays_o.dtype, device=rays_o.device)
        # ray_pts = rays_o
        # mask_outbbox = torch.zeros((n_rays, N_samples), dtype=torch.bool, device=rays_o.device)
        min_condition = ray_pts < xyz_min.view(1,1,3)
        min_condition_all = min_condition[...,0] | min_condition[...,1] | min_condition[...,2]
        max_condition = ray_pts > xyz_max.view(1,1,3)
        max_condition_all = max_condition[...,0] | max_condition[...,1] | max_condition[...,2]
        mask_outbbox = min_condition_all | max_condition_all
        


        return ray_pts, mask_outbbox.detach().clone()

# -----------------------------------------------------------------------------------------------------------------
# modified ------------------------------------------------------------------------
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, use_mask=True, **render_kwargs):
        '''Volume rendering
        @rays_o:   [N, 3] the starting point of the N shooting rays.
        @rays_d:   [N, 3] the shooting direction of the N rays.
        @viewdirs: [N, 3] viewing direction to compute positional embedding for MLP.
        '''
        assert len(rays_o.shape)==2 and rays_o.shape[-1]==3, 'Only suport point queries in [N, 3] format'
        #assert rays_o.shape[-1]==3, 'Only suuport point queries in [N, 3] format'
        ...
        ray_pts, ray_id, step_id, N_samples, mask_inbbox = self.sample_ray(
                rays_o=rays_o, rays_d=rays_d, use_mask=use_mask, **render_kwargs)
        interval = render_kwargs['stepsize'] * self.voxel_size_ratio

        mask_final=mask_inbbox.reshape(-1)
        # skip known free space
        if self.mask_cache is not None: # True
            mask = self.mask_cache(ray_pts)
            ray_pts = ray_pts[mask]
            ray_id = ray_id[mask]
            step_id = step_id[mask]
            if mask_final is None:
                mask_final = mask.detach()
            else:
                temp = mask_final.detach().clone()                
                mask_final[temp] = (mask_final[temp] & mask.detach()).clone()
        ...
        if self.rgbnet is None:
            ...
            rgb_feat = torch.cat([vox_emb, ray_pts, viewdirs_emb], -1)

            max_freq = rgb_feat.shape[1]
            # if global_step is not None:
            #     viewdirs_emb_w = (1-torch.cos(
            #                         math.pi * torch.clamp(
            #                             (global_step*max_freq/self.iter_with_max_view_embedding)
            #                             -torch.arange(max_freq) + 3,
            #                             0,1
            #                             )
            #                         )
            #                     )/2
            #     rgb_feat=rgb_feat*viewdirs_emb_w.view(1,-1)
        ...
        rgb_marched = segment_coo(
                ...
                out=torch.zeros([N, 3]),
                reduce='sum')
        if render_kwargs.get('rand_bkgd', False) and global_step is not None: # False
            rgb_marched += (alphainv_last.unsqueeze(-1) * torch.rand_like(rgb_marched))
        else:
            rgb_marched += (alphainv_last.unsqueeze(-1) * render_kwargs['bg'])
        s = (step_id+0.5) / N_samples

        # aggregate aperture rays --------------------------------
        
        ret_dict.update({
            'alphainv_last': alphainv_last, # flatten pixel, aperture
            'weights': weights, # flatten pixel, aperture, 255 sample 
            'rgb_marched': rgb_marched, # flatten pixel, aperture
            'raw_alpha': alpha, # flatten pixel, aperture, 255 sample 
            'raw_rgb': rgb, # flatten pixel, aperture, 255 sample  x 3
            'ray_id': ray_id, # flatten pixel, aperture, 255 sample 
            'n_max': N_samples, # 255
            's': s, # flatten pixel, aperture, 255 sample 
            'mask_final': mask_final # flatten pixel, aperture, 255 sample 
        })
