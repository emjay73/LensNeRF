...
def _minify(basedir, factors=[], resolutions=[]):
    # gyojung custom
    if factors != []:
        raise RuntimeError('"_minify" function cannot get "factors" argument!')
    ...
    # gyojung custom
    src_dir = os.path.join(basedir, "images")
    src_images = os.listdir(src_dir)
    
    for r in resolutions:
        dst_dir = os.path.join(basedir, "images_{}x{}".format(r[1], r[0]))
        os.makedirs(dst_dir, exist_ok=True)
        
        for im_name in src_images:
            im = Image.open(os.path.join(src_dir, im_name))
            im_re = im.resize((r[1], r[0]), Image.LANCZOS)
            im_re.save(os.path.join(dst_dir, im_name.replace('.jpg', '.png')))
